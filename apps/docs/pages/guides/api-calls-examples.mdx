...
export const meta = {
  id: 'value-chain-api',
  title: 'API examples',
  description: 'How to make calls to API endpoints exposed by dijets node.',
}
...
Below is an example call made to the admin api endpoint on dijets node running locally.
...
```
The admin api uses the endpoint /ext/admin

<Admonition type="info">
The Admin API is disabled by default for security reasons. To run a node with the Admin API
enabled, use [config flag `--api-admin-enabled=true`](/docs/guides/nodes/node-flags-configs#apis).
...
The admin api method `admin.alias` assigns an alias to the specified endpoint, which is a different endpoint from the one exposed by dijets nodes. The original endpoint will still
work. This change only affects the node whose address is used in the call; other nodes will not know about this alias.

**Signature:**

The signature used for the alias method is
