...
(You can use [Dijets-Up](/guides/getting-started/tutorials/create-local-network) to test things locally.) Dijets TestNet is
...
[DijetsJS](/guides/dijetsjs). Mnemonic phrases enable us to encode
...
We can get 1 DJT from Dijets faucet. Paste the address into
[Dijets faucet website](https://faucet.dijets.io). These DJT are for Dijets
Testnet and have no monetary value.

![Requesting DJT](/docs/img/dijets-faucet.png)

The faucet will send 1 DJT to the address and return a transaction ID
(txID). This txID can be used with the Dijets Testnet Explorer to learn more about
the transaction.

![Receiving DJT](/docs/img/dijets-faucet-txid.png)
...
The faucet sent 1 DJT to the first address we generated. Let's send DJT from
...
Wallet](https://wallet.dijets.io). We'll see that it has the DJT balance and
...
The [Dijets Faucet](https://faucet.dijets.io) can send DJT to Value Chain or Utility Chain
