...
export const meta = {
  id: 'multisig-utxos',
  title: 'Multisig UTXOs with DijetsJs',
  description: 'Multisig Unspent Transaction Outputs.',
  sidebar_label: 'Multisig UTXOs',
}

# Introduction

An account on a chain that follows the UTXO model much like Bitcoin doesn't have the
...
Suppose, account Alice wants to send 1.3 DJT to account Bob, then it has to include all
...
below, a total of 1.72 DJT is getting unlocked in inputs, therefore we have
also created a change output for the excess amount (0.41 DJT) to the sender's
address. The remaining amount after being consumed by the outputs like
receiver's and change output, is burned as fees (0.01 DJT).

![multisig UTXOs 1](/docs/img/multi-utxos-djtx.png)
...
On Dijets, we can even create cross-chain outputs. This means that we can do
...
![multisig UTXOs 1](/docs/img/multi-utxos-djtx2.png)

## UTXOs on Utility Chain

Dijets Utility Chain is an instance of Ethereum Virtual Machine. Unlike the Value and Method Chain, UTXOs can't be
...
But we can export UTXOs with one or multiple owners to Utility Chain and then import
...
Create an `.env` file for storing sensitive information which we can't make
public like the private keys or the mnemonic. Here are the sample private keys,
which you should not use. You can create a new account on [Dijets
Wallet](https://wallet.dijets.io) and paste the mnemonic here for
demonstration.
...
To make API calls to the Dijets network and to its Ternary Chain Ledgers of
...
In order to sign transactions with our private keys, we will use the DijetsJs
...
But since we can't use UTXOs on Dijets EVM instance, the Utility Chain, we cannot directly import them either.
...
As explained earlier, we do not have UTXOs on Utility Chain. Therefore we cannot make
...
EVM Inputs are required when we want to export assets from Utility Chain. In the
following function, `addresses` is the array of Buffer addresses but for
`Utility Chain Export Transactions`, a hex address is also appended at last.
...
- Export from Utility Chain (C.export == true) - These types of transactions cannot
...
are only including outputs containing `DJT` assets. These conditions are
...
Utility Chain and `EVMOutput` should be used as outputs while creating an import
...
- Multi-sig output of value 0.5 DJT with threshold 2 and owners represented by
...
Run this file using `node examples/sendBaseTx.js`, note the txID in the output,
and look for it in the Dijets Unified explorer.

## Export Multi-Sig UTXO From X to Method Chain

Now we will look into exporting assets from the Value to Method Chain. It will be similar
...
Method Chain. These addresses will be required for signing `importTx` on Method Chain.
...
imported on Method Chain) and 2 `0.005` imported outputs (1 multi-sig and 1
...
Now let's try using the multi-sig UTXOs exported from Value Chain to Method Chain to
...
const { NodeIDStringToBuffer, UnixNow } = require("dijets/dist/utils")
...
output, and look for it in the Dijets Unified explorer.
