...
1. You have completed the tutorial for ["Create a Local Dijets Network"](/guides/getting-started/tutorials/create-local-network)

1. You have Go language installed on your system with $GOBIN set to be [in your path.](/guides/getting-started/tutorials/funding-test-accounts#setting-bin-in-your-path)
...
In [Create a Local Test Network](/guides/getting-started/tutorials/create-local-network), you learnt
...
sign transactions locally using [DijetsJS](/guides/dijetsjs).
...
```text
curl --location --request POST '127.0.0.1:9650/ext/bc/V' \
--header 'Content-Type: application/json' \
--data-raw '{
    "jsonrpc":"2.0",
    "id"     :1,
...
the [djt.importKey API](#avmimportkey).

```text
curl --location --request POST '127.0.0.1:9650/ext/bc/C/djt' \
--header 'Content-Type: application/json' \
--data-raw '{
    "method": "djt.importKey",
    "params": {
        "username": "PenXenOleFren",
        "password": "password",
        "privateKey":"PrivateKey-ewoqjP7PxY4yr3iLTpLisriqt94hdyDFNgchSxGGztUrTXtNN"
