...
export const meta = {
  id: 'dynamic-gas-fee',
  title: 'Adjusting Gas Prices during high network activity',
  description: 'Learn how to adjust Gas Prices when the network is busy',
  sidebar_label: 'Adjusting Gas Prices',
}

### Introduction

This tutorial is designed to help users adjust the priority fee and max fee cap for a transaction during periods of high network activity.
Follow this tutorial to take full advantage of the benefits of dynamic fee transactions.

Occassionally during periods of extremely high network activity, transactions can either remain
pending for a very long period or instantly get a failed transaction
notification. This can lead to confusion and frustration for users, especially the ones
unfamiliar with Gas Fees and how they are calculated.

## This tutorial will help you if:

- Your transaction has been pending for a long time, and you don't know what to do
- Your transaction has failed, with an error such as `transaction was underpriced`
- It's your first transaction, and you want to be sure about any potential issues

You can directly skip to
the [solution](#adjusting-gas-fees-before-submitting-the-transaction) if you don't wish to read the
entire tutorial and key concepts.

<Admonition type="info">
Gas fees should only be adjusted when your transactions are failing or pending for a long period due to
high network activity or heavy network congestion when fees are frequently fluctuating. Such adjustments are not required during normal network activity days.
Wallets like Decypher and MetaMask will automatically do that for you.
</Admonition>

## Keywords and Concepts

The amount of computation used by a transaction is measured in units of `gas`.
Each unit of gas is paid for in DJT at the `gas price` for the transaction. The
...
info, read [this](transaction-fees#dynamic-fee-transactions).
...
network utilisation. This window has a target utilisation, which is currently
...
Consider the following examples (here Gwei or nDJT equates to one-billionth of 1 DJT)
...
prioritise transaction A since thats the transaction thats willing to pay an higher effective tip.

## Primary reasons for Trasnaction Pending or Failing?
...
price that the network supports (as of now, it's 25 nDJT or Gwei). Although the
...
base fee for it to be included in the block or wait for the network activity to get a bit lower before trying again.

<Admonition type="info">
You can use Decypher Extension which automatically adjusts the Gas Fees based on Dijets Utility Chain's Gas Tracker. 
Or you can manually make the fee adjustments through wallets like MetaMask.
</Admonition>
...
1. Let's create a sample transaction on Dijets Mainnet, in which we will be
   sending 0.1 DJT to a receiver using the MetaMask. By clicking **Next** we
   can review gas fees and the amount which we want to send.

1. On the review page, you can see the estimated priority and max fee for this
   transaction. Click on **EDIT**, to adjust these fees according to network
   requirements.

1. You can edit the priority fee (gas tip cap) and max fee (max
   fee cap) using the advanced options or you can use MetaMask's inbuilt gas
   estimation. You can estimate the max fee as shown on
   [Dijets Utility Chain Explorer](https://utility-explorer.swedencentral.cloudapp.azure.com) which represents the average max
   fee over the last 5 seconds.

1. If the network activity is high, you have to edit the priority and max fees
   accordingly, as shown on Dijets Utility Chain Explorer. Consider the example below, where the
   average max fee is 31.25 Gwei (nDJT).

   ![Dijets Utility Chain Gas Tracker](/docs/img/gas-tracker.png)

1. It is recommended to set the max fee cap as the maximum price that you are
   ...
   should not have to worry about getting overcharged. Edit the max
   ...
   between 0 and 40 Gwei. The more the priority fee the faster the transaction is.
   As an example, let's set this to 2 Gwei. You can also rely on
   MetaMask's automatic estimate instead of opting for advanced options. Now,
   save and confirm the transaction.

1. After submitting the transaction, even if the base fee has decreased, you
...
MetaMask. Click on the **Speed Up** button, to edit
your priority and max fee. By default, the new transaction has slightly more
priority and max fee (say 10% more than the previous), but you can edit the value as required.

You can look at Utility Chain Explorer's gas tracker to get an estimate about the average
