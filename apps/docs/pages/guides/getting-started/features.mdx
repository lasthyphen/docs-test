...
Dijets is a fixed cap asset, with a max of 100M DJTX in circulation. It employs Satoshi’s UTXO model for single-use credentials to facilitate payments, rather than an account model. Finally, Dijets structures transactions like Segwit, without backward compatibility issues, and uses the Bech32 address format to minimise errors and maximise readability. (human readable part of an address reads "dijets")
