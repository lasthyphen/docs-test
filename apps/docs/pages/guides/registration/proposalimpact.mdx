...
* Anything that adds or removes a non-trivial amount of DJT from the Treasury Wallet is likely to be marked as `high` impact.
...
* Proposals that may lead to later decisions that add or remove a non-trivial amount of DJT from the Treasury Wallet is likely to be marked as `medium` impact.
