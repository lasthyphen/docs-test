...
Node](/docs/guides/nodes/run-dijets-node-manually) and are familiar with
[Dijets
architecture](/docs/guides/getting-started/introduction). In this
tutorial, we use [DijetsJS](/docs/guides/dijetsjs) and
[Dijets Postman
...
Calling
[`platform.getPendingValidators`](/docs/node-apis/method-chain-api#methodgetpendingvalidators)
...
[`method.getCurrentValidators`](/docs/node-apis/method-chain-api#methodgetpendingvalidators)
...
We can also add a node to the validator set using [DijetsJS](/docs/add-validator-with-dijets).
...
To learn more about encoded addresses, click [here](/docs/guides/dijetsjs/keys-management#encode-bech32-addresses).
...
example, `BN(1e12)` which is 10,000 DJTX.
...
- Network setting should be to a Mainnet node, either [a local node on
  ...
  [Dijets Mainnet API
  server](/docs/guides/public-api#using-the-public-api-nodes)
  where `` should be used for the `ip`.
- `const networkID: number = 1` based on [this](/docs/guides/dijetsjs/keys-management#encode-bech32-addresses).
