...
Dijets Primary
Network 
validates Dijets Ternary Chain Ledgers. In this
tutorial, we’ll add a node to the Primary Network on Dijets.

The Method Chain manages metadata on Dijets. This includes tracking which nodes
...
our [Qowalts](https://redirect.dijets.io/#/#dijets-support:dijets.org) to ask questions.

</Admonition>

## Requirements

You've completed [Run an Dijets
Node](/guides/nodes/run-dijets-node-manually) and are familiar with
[Dijets
architecture](/guides/getting-started/introduction). In this
tutorial, we use [DijetsJS](/guides/dijetsjs) and
[Dijets Postman
collection](https://github.com/Dijets-Inc/dijets-postman-collection) to help us
make API calls.
...
First, we show you how to add your node as a validator by using [Dijets Wallet](https://wallet.dijets.io).

### Retrieve the Node ID

Get your node’s ID by calling [`info.getNodeID`](/docs/guides/node-apis/info-api#infogetnodeid):
...
Open [the wallet](https://wallet.dijets.io), and go to the `Earn` tab. Choose
`Add Validator` under the `Validate` section.

Fill out the staking parameters. They are explained in more detail in [this
doc](staking). When you’ve filled in all the staking parameters
...
least 2,000 DJT on Mainnet (1 DJT on Dijets TestNet Testnet).

You should see a success message, and your balance should be updated.

Calling
[`platform.getPendingValidators`](/docs/guides/node-apis/method-chain-api#methodgetpendingvalidators)
...
[`method.getCurrentValidators`](/docs/guides/node-apis/method-chain-api#methodgetpendingvalidators)
...
We can also add a node to the validator set using [DijetsJS](/guides/dijetsjs).
...
[**`examples/platformvm`**](https://github.com/Dijets-Inc/dijetsjs/tree/master/examples/platformvm)
folder to view the source code for the examples scripts.

We will use the
[**`buildAddValidatorTx.ts`**](https://github.com/Dijets-Inc/dijetsjs/blob/master/examples/platformvm/buildAddValidatorTx.ts)
script to add a validator. To learn more about the `buildAddValidatorTx` API,
please click
[here](https://github.com/Dijets-Inc/dijetsjs-docs/blob/main/classes/api_platformvm.platformvmapi.md#buildaddvalidatortx).
...
and replace this with a private key that you control. You can use [this code to generate a new key](https://github.com/Dijets-Inc/dijetsjs/blob/master/examples/platformvm/createKeypair.ts).
...
The following settings work when using a local node started with [`--network-id=testnet`](../..//docs/guides/nodes/node-flags-configs#network-id):
...
However, to connect directly to the [Dijets API
server](public-api-server), the following changes are
needed:
...
Depending on the networkID passed in when instantiating an `Dijets` object in
the code, the encoded addresses used will have a distinctive Human Readable
Part(HRP) per network.

_Example Address: 5 - X-`dijets`19rknw8l0grnfunjrzwxlxync6zrlu33yxqzg0h_

For Dijets TestNet Testnet, 5 is the correct value to use.

To learn more about encoded addresses, click [here](/guides/dijetsjs/keys-management#encode-bech32-addresses).
...
[`info.getNodeID`](/docs/guides/node-apis/info-api#infogetnodeid).
...
example, `BN(1e12)` which is 10,000 DJT.
...
time to add a validator to the Dijets TestNet Network.
...
[`getTxStatus.ts`](https://github.com/Dijets-Inc/dijetsjs/blob/master/examples/platformvm/getTxStatus.ts)
...
by using the
example:[`getPendingValidators.ts`](https://github.com/Dijets-Inc/dijetsjs/blob/master/examples/platformvm/getPendingValidators.ts).
Just change the [network settings](#network-setting) to meet Dijets TestNet requirements
...
Network. The staked DJT and the rewards, if any, will be returned to
`pAddressStrings`.
...
[`createKeypair.ts`](https://github.com/Dijets-Inc/dijetsjs/blob/master/examples/platformvm/createKeypair.ts)
example script along with [Dijets TestNet Network Settings](#network-setting).
...
- Network setting should be to a Mainnet node, either [a local node on
  Mainnet](/guides/nodes/node-flags-configs#network-id) or
  [Dijets Mainnet API
  server](/guides/public-api#using-the-public-api-nodes)
  where `` should be used for the `ip`.
- `const networkID: number = 1` based on [this](/guides/dijetsjs/keys-management#encode-bech32-addresses).
