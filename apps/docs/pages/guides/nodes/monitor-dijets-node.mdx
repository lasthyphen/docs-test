...
export const meta = {
  ...
  title: 'Dijets Node Config and Flags',
  description: "Run Dijets Node with different configurations and flags",
}
...
:::tip
[Install Script](/guides/nodes/run-dijets-node-with-installer) creates the node config
file at `~/.dijetsnodego/configs/node.json`. No default file is created if
[DijetsNodeGo is built from source](/guides/nodes/run-dijets-node-manually), you
...
`"2K33xS9AyP9oCDiHYKVrHe7F54h2La5D8erpTChaAhdzeSu2RX"` to `"FoundationX"`. Chain
...
`{subnetID}.json` config file under `--subnet-config-dir`. DijetsNodeGo loads
...
Defaults to `$HOME/.dijetsnodego/configs/subnets`. If the flag is set explicitly,
...
`$HOME/.dijetsnodego/configs/subnets/p4jUwqZsA2LuSftroCd3zb4ytH8W99oXKuKVZdsty7eQ3rXD6.json`.
...
to `720,000,000,000,000,000` nDJT. This can only be changed on a local network.
...
`~/.dijetsnodego/configs/vms/aliases.json`. This flag is ignored if
