...
DJT. For a node to
validate
a blockchain on Dijets, it must stake DJT.

## Staking Parameters on Dijets

When a validator is done validating
it receives back the DJT tokens it staked. It may receive a reward for helping
...
You do not need to have DJT funds on your validating node. In fact, it's best
