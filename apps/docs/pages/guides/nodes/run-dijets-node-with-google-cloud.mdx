...
export const meta = {
  id: 'nodes-run-dijets-node-with-google-cloud',
  title: 'Run a Dijets Node with Google Cloud Services',
  description: "How to configure and run a Dijets Node on Google Cloud",
}
...
- `Items` highlighted in this manner are GCP parlance and can be searched for
...
We will create and utilise a single GCP `Project` for deployment of all resources.
...
Dijets requires that a validator communicate outbound on the same public IP
...
- P2P INGRESS for DJT Peers (TCP 9651)

These are obviously just default ports and can be tailored to your needs as you see fit.
...
You will get some free credit to run a trial, the trial is feature complete but
...
organization. Let's use the name `my-dijets-nodes` for the sake of this setup.

### Terraform State

Terraform uses state files to compose a differential between current
...
We have have provided a rudimentary terraform construct to provision a node on which
to run Dijets which can be found
[here](https://github.com/Dijets-Inc/dijets-terraform-construct).
...
3. Under the `projects` directory, rename the `my-dijets-project` directory to
...
If you do not wish to use cloud storage to persist terraform state then simply
...
1. In a terminal which is able to execute the `tf` binary, `cd` to the
   ~`my-dijets-project` directory that you renamed in step 3 of `Terraform
   Configuration`.
