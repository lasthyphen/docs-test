...
export const meta = {
  id: 'nodes-monitor-dijets-node',
  title: 'DijetsNodeGo Configuration',
  description: "Config and Flags for Dijets Node",
}
...
[Install Script](run-dijets-node-with-installer-script) creates the node config
file at `~/.dijetsnodego/configs/node.json`. No default file is created if
[DijetsNodeGo is built from source](run-dijets-node-manually), you
...
See [here](/docs/guides/node-apis/admin-api) for more information.

### `--api-auth-required` (boolean)

If set to `true`, API calls require an authorization token. Defaults to `false`.
See [here](/docs/guides/node-apis/auth-api) for more information.
...
[here](/docs/guides/node-apis/auth-api) for more information.

### `--api-health-enabled` (boolean)

If set to `false`, this node will not expose the Health API. Defaults to `true`. See
[here](/docs/guides/node-apis/health-api) for more information.
...
will be used to bootstrap the current Dijets state. An example setting of
...
categories: network upgrades and optional chain configurations. DijetsNodeGo
...
Specifies the directory that contains chain configs. Defaults to `$HOME/.dijetsnodego/configs/chains`.
If this flag is not provided and the default directory does not exist,
DijetsNodeGo will not exit since custom configs are optional. However, if the
flag is set, the specified folder must exist, or DijetsNodeGo will exit with an
...
`~/.dijetsnodego/configs/chains/aliases.json`. This flag is ignored if
...
standard networks (Mainnet, Dijets TestNet Testnet), or when `--genesis-content` is
...
For an example of a JSON representation of genesis data, see [genesis_local.json](https://github.com/Dijets-Inc/dijetsnodego/blob/master/genesis/genesis_local.json).
...
node and the Dijets network. This argument specifies the port that the HTTP
...
DijetsNodeGo supports collecting and exporting [OpenTelemetry](https://opentelemetry.io/) traces.
...
The fraction of traces to sample. If >= 1, always sample. If equals or less than 0, never sample.
Defaults to `0.1`.
...
Dijets uses Proof of Stake (PoS) as sybil resistance to make it prohibitively
...
not be disabled on public networks (`Dijets TestNet` and `Mainnet`).
...
Dijets uses two-way authenticated TLS connections to securely connect nodes.
...
`$HOME/.dijetsnodego/staking/staker.crt`. This flag is ignored if
...
`$HOME/.dijetsnodego/staking/staker.key`. This flag is ignored if
...
`{subnetID}.json` config file under `--subnetwork-config-dir`. DijetsNodeGo loads
configs for Subnets specified in 
`--track-subnetworks` parameter.

Full reference for all configuration options for a Subnet can be found in a
separate [Subnet Configs](subnetwork-configs) document.

#### `--subnetwork-config-dir` (`string`)

Specifies the directory that contains Subnet configs, as described above.
Defaults to `$HOME/.dijetsnodego/configs/subnetworks`. If the flag is set explicitly,
the specified folder must exist, or DijetsNodeGo will exit with an error. This
flag is ignored if `--subnetwork-config-content` is specified.

Example: Let's say we have a Subnet with ID
`p4jUwqZsA2LuSftroCd3zb4ytH8W99oXKuKVZdsty7eQ3rXD6`. We can create a config file
under the default `subnetwork-config-dir` at
`$HOME/.dijetsnodego/configs/subnetworks/p4jUwqZsA2LuSftroCd3zb4ytH8W99oXKuKVZdsty7eQ3rXD6.json`.
...
:::note
Some of these parameters can only be set on a local or private network, not on Dijets TestNet Testnet or Mainnet
:::
...
Transaction fee, in nDJT, for transactions that create new assets. Defaults to
`10000000` nDJT (.01 DJT) per transaction. This can only be changed on a local
network.

#### `--create-subnetwork-tx-fee` (int)

Transaction fee, in nDJT, for transactions that create new Subnets. Defaults to
`1000000000` nDJT (1 DJT) per transaction. This can only be changed on a local
network.

#### `--create-blockchain-tx-fee` (int)

Transaction fee, in nDJT, for transactions that create new blockchains.
Defaults to `1000000000` nDJT (1 DJT) per transaction. This can only be
...
Defaults to `25000000000` (25 DJT) on Mainnet. Defaults to `5000000` (.005
DJT) on Test Net. This can only be changed on a local network.
...
The minimum stake, in nDJT, required to validate the Primary Network. This can
only be changed on a local network.
...
The maximum stake, in nDJT, that can be placed on a validator on the primary
network. Defaults to `3000000000000000` (3,000,000 DJT) on Mainnet. This
...
The maximum stake supply, in nDJT, that can be placed on a validator. Defaults
to `111,000,000,000,000,000` nDJT. This can only be changed on a local network.

#### `--tx-fee` (int)

The required amount of nDJT to be burned for a transaction to be valid on the
Value Chain, and for import/export transactions on the Method Chain. This parameter
...
`1,000,000` nDJT per transaction.
...
at this directory. Defaults to the `$HOME/.dijetsnodego/profiles/`.
