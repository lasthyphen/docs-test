...
DJT account balances. As with other `yarn` scripts you can pass in a
...
Prints a list of accounts and their corresponding DJT balances on the
...
The first account with DJT balance is the same account we used in previous tutorials too.
...
API](/guides/public-api) for the provider. For a local
