...
export const meta = {
  id: 'truffle-dijets-uc',
  title: 'Use Truffle with Dijets Utility Chain',
...
Dijets's Utility Chain, which is an instance of the EVM.

## Recommended Knowledge

- Completed the tutorial [Run a Dijets Node](nodes/run-dijets-node-manually).
...
- [Dijets-Up](https://github.com/Dijets-Inc/dijets-up) is a tool for
  running a local Dijets network. It's similar to Truffle's
...
[Dijets-Up](https://github.com/Dijets-Inc/dijets-up)
allows you to spin up a private test network deployments. 
...
[DijetsJS](/guides/dijetsjs) which is used for cross chain
swaps.
...
Lastly, create a boilerplate truffle project by initialising it with:
...
`gasPrice` and `gas` to the appropriate values for the Dijets Utility Chain.
...
available account provided by your Utility Chain client as the `from` address used
...
You'll to have a minimum of `0.15` DJT unlocked in your account to cover the cost
...
`Utility Chain`/`Coreth` configs. The Dijets Network Runner currently does not
...
Network/Manually](/guides/quickstart/create-a-local-test-network.md#manually) and
[Utility Chain Configs](/guides/nodes/node-flags-configs#utility-chain-configs).
...
Now you have the tools you need to launch a local Dijets network, create a
