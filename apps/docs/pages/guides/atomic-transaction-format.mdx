...
export const meta = {
  ...
  title: 'Atomic Transaction Format',
  description: 'The format of Atomic Transactions in Dijets',
  sidebar_label: 'Overview',
}

This reference document explains how atomic transactions are serialised on Dijets 
Utility Chain - an instance of Ethereum Virtual Machine. The document uses the [primitive
serialisation](serialisation-primitives) format for packing and
[secp256k1](cryptographic-primitives#cryptography-in-dijets-virtual-machine)
...
- **`Amount`** is the amount of the asset to be transferred (specified in nDJT
  for DJT and the smallest denomination for all other assets).
...
A
[secp256k1](cryptographic-primitives.md#cryptography-in-dijets-virtual-machine)
...
- **`Addresses`** is a list of unique addresses that correspond to the private
  keys that can be used to spend this output. 
...
- **`networkID`** is an int that defines which Dijets network this
  transaction is meant to be issued to. This could refer to Mainnet, Dijets TestNet, etc.
...
- **`Output`** is the output object that created this utxo. The serialisation of
