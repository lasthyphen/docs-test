...
The [Value Chain](/guides/getting-started/introduction#value) is
...
`platform.importDJT`](method-chain-api#platformimportdjtx) to complete the transfer.
...
called, you must call the Method Chain’s [`platform.exportDJT`](method-chain-api#platformexportdjtx) or
...
- `to` is the address the DJT is sent to. This must be the same as the `to` argument in the
  corresponding call to the Method Chain’s `exportDJT` or Utility Chain's `export`.
...
- Sends `amount` units of asset with ID `assetID` to address `to`. `amount` is denominated in the
  smallest increment of the asset. For DJT this is 1 nDJT (one billionth of 1 DJT.)
