...
balance of first class Dijets Native Tokens on the Utility Chain (excluding DJT, which must be fetched
with `eth_getBalance`).
...
documentation](/guides/getting-started/tutorials/transaction-fees#utility-chain-fees).
...
Send DJT from the Utility Chain to Value Chain or Method Chain. After calling this method, you must call the
Value Chain's [`avm.import`](value-chain#avmimport) or Method Chain's
[`platform.import`](method-chain#methodimportdjtx) with assetID `DJT` on the Value Chain to complete
the transfer.
...
Finalize the transfer of a non-DJT or DJT from Value Chain or Method Chain to the Utility Chain. Before this
method is called, you must call the Value Chain’s [`avm.export`](value-chain-api#avmexport) or Method Chain’s
[`platform.exportDJT`](method-chain-api#platformexportdjtx) with assetID `DJT` to initiate the transfer.
...
Finalize a transfer of DJT from the Value Chain or Method Chain to the Utility Chain. Before this method is
...
- `sourceChain` is the ID or alias of the chain the DJT is being imported from. To import funds
  from the Value Chain, use `"X"`; for the Method Chain, use `"P"`.
- `baseFee` is the base fee that should be used when creating the transaction. If omitted, a
  suggested fee will be used.
- `username` is the user that controls the address that transaction will be sent from.
