...
export const meta = {
  id: 'issuing-api-calls',
  title: 'Issue API Calls',
  description: 'Learn how to make calls to an API exposed by dijets node',
}

Dijets API is a specifically-tailored collection of JSON-RPC API endpoints built to support 
...
If you're making RPC calls to remote nodes, then instead of `127.0.0.1` you
...
The Primary Network consists of three chains: X, P and Utility Chain. As those chains
...
Utility Chain and many Subnetworks run a version of the EthereumVM (EVM). EVM exposes its
...
When using the [Public API](/docs/guides/node-apis/public-api) or another host
that supports HTTPS, use `https://` or `wss://` instead of `http://` or `ws://`.

Also, note that the [public API](/docs/guides/node-apis/public-api) only
...
Suppose we want to call the `getTxStatus` method of the [Value Chain
API](Value Chain.md). The Value Chain API documentation tells us that the endpoint for
this API is `/ext/bc/Value`.
