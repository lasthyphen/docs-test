...
flag](../../..//docs/guides/nodes/node-flags-configs)`--api-auth-required`. If the node is
...
(that is the Value Chain) and `/ext/info` (that is the [info API](info-api).)
...
For example, to call [`info.peers`](info-api#infopeers) with this token:
